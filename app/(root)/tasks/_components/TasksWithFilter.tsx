...
export const TasksWithFilter = ({ tasks, children }: ITasksWithFilterProps) => {
  const { storedTasks, dispatch } = useTaskStore((state) => state);

  useEffect(() => dispatch({ type: SET_TASKS, payload: tasks }), []);

  const { filteres, filterByStatus, filterByCategory, resetFilters, filterTask } = useFilter();
  ...
  const onDragEndHandler = async (e: DragEndEvent) => {
    ...
    updateTaskPositionActionCreator(storedTasks, e.active.id, e.over?.id)(dispatch);
  };
